...
function normalizeHexSearchResults(hexagons: any[]): HexResult[] {
  ...
  for (const hex of hexagons) {
    ...
    normalized.push({
      ...
      city_id: h3,
      h3_index: h3,
      name: hex.admin_name || hex.name || hex.parent_city_name || hex.city || 'Unknown',
          ...
          setSelectedCity((prev) =>
            prev && (prev.city_id || prev.id) === (city.city_id || city.id)
              ? {
                  ...prev,
                  ...detail,
                  name: detail.admin_name || detail.parent_city_name || prev.name,
                  city: detail.parent_city_name || prev.city,
                  parent_city_name: detail.parent_city_name || prev.parent_city_name,
